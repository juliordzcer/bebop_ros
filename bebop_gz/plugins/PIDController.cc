    ...
    EventManager &/*_eventMgr*/)
{
  ...
  {
    gzerr << "PIDController plugin should be attached to a model "
  ...
  if (sdfClone->HasElement("Kp_pos"))
  {
    controllerParameters.Kp_pos =
        math::eigen3::convert(sdfClone->Get<math::Vector3d>("Kp_pos"));
  }
  else
  {
    gzerr << "Please specify Kp_pos for PIDController.\n";
    return;
  }
  
  if (sdfClone->HasElement("Ki_pos"))
  {
    controllerParameters.Ki_pos =
        math::eigen3::convert(sdfClone->Get<math::Vector3d>("Ki_pos"));
  }
  else
  {
    gzerr << "Please specify Ki_pos for PIDController.\n";
    return;
  }

  if (sdfClone->HasElement("Kd_pos"))
  {
    controllerParameters.Kd_pos =
        math::eigen3::convert(sdfClone->Get<math::Vector3d>("Kd_pos"));
  }
  else
  {
    gzerr << "Please specify Kd_pos for PIDController.\n";
    return;
  }

  if (sdfClone->HasElement("Kp_att"))
  {
    controllerParameters.Kp_att =
        math::eigen3::convert(sdfClone->Get<math::Vector3d>("Kp_att"));
  }
  else
  {
    gzerr << "Please specify Kp_att for PIDController.\n";
    return;
  }
  
  if (sdfClone->HasElement("Ki_att"))
  {
    controllerParameters.Ki_att =
        math::eigen3::convert(sdfClone->Get<math::Vector3d>("Ki_att"));
  }
  else
  {
    gzerr << "Please specify Ki_att for PIDController.\n";
    return;
  }

  if (sdfClone->HasElement("Kd_att"))
  {
    controllerParameters.Kd_att =
        math::eigen3::convert(sdfClone->Get<math::Vector3d>("Kd_att"));
  }
  else
  {
    gzerr << "Please specify Kd_att for PIDController.\n";
    return;
  }

  this->velocityController = VelocityControllerPID::MakeController(
      controllerParameters, vehicleParams);

  if (nullptr == this->velocityController)
  {
    gzerr << "Error while creating the VelocityControllerPID\n";
    return;
  }
  ...
  this->node.Subscribe(topic, &PIDController::OnTwist, this);
  gzmsg << "PIDController subscribing to Twist messages on ["
         << topic << "]" << std::endl;

  std::string enableTopic{this->robotNamespace + "/" + this->enableSubTopic};
  this->node.Subscribe(enableTopic, &PIDController::OnEnable,
                       this);
  gzmsg << "PIDController subscribing to Boolean messages on ["
...
//////////////////////////////////////////////////
math::Inertiald PIDController::VehicleInertial(
...
//////////////////////////////////////////////////
void PIDController::PreUpdate(
    const UpdateInfo &_info,
    EntityComponentManager &_ecm)
{
  GZ_PROFILE("PIDController::PreUpdate");
...
//////////////////////////////////////////////////
void PIDController::PublishRotorVelocities(
    EntityComponentManager &_ecm,
...
GZ_ADD_PLUGIN(PIDController,
                    System,
                    PIDController::ISystemConfigure,
                    PIDController::ISystemPreUpdate)

GZ_ADD_PLUGIN_ALIAS(
    PIDController,
    "gz::sim::systems::PIDController")
